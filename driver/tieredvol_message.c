...
found:
	...
	if (argc == 1 && strcmp(argv[0], "show_log") == 0) {
		...
		while (kfifo_out(&tv_log_fifo, &entry, sizeof(entry))) {
			pr_info("tieredvol: LOG %s %s: %s\n",
				entry.level == TV_LOG_ERR ?
					"ERR" :
				entry.level == TV_LOG_WARN ?
					"WRN" :
					"INF",
				entry.event_type == TV_LOG_STALE ?
					"STALE" :
				entry.event_type == TV_LOG_RECOVER ?
					"RCVR" :
				entry.event_type == TV_LOG_MIRROR ?
					"MIRR" :
				entry.event_type == TV_LOG_CONFIG ?
					"CONF" :
		...
		for (i = 0; i < ctx->ndisks; i++) {
			if (ctx->deg.degraded[i]) {
				...
				pr_info("tieredvol: disk[%d] %s cleared from DEGRADED\n",
					i, ctx->meta.disk_names[i]);
				tv_log(TV_LOG_INFO, i, TV_LOG_IO,
				       "CLEARED degraded");
			}
		}
	...
	if (argc >= 2 && strcmp(argv[0], "start_rebuild") == 0) {
		...
		if (kstrtou32(argv[1], 10, &seg_idx) ||
			...
			return -EINVAL;
		if (argc >= 3) {
			if (kstrtou64(argv[2], 10, &max_bytes) ||
			    max_bytes == 0)
				return -EINVAL;
		}
		...
		pr_info("tieredvol: rebuild started seg%u %llu bytes\n", seg_idx,
			ctx->rebuild.total);
		tv_log(TV_LOG_INFO,
		       ctx->meta.segments[seg_idx].mirror_disk,
